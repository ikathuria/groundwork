...
description: |
  ...
  (the canonical scene spec consumed by the AR/3D viewer in `components/ar/`)
  and files a copy back into the wiki. Pass 4 does NOT generate any HTML —
  the live AR viewer is the only renderer.
---
...
- `hypotheses/<slug>/wiki/plans/ar-v<n>.md` — wiki-filed copy of `ar.json` (thin wrapper: `type: ar-spec`, `derived_from: plan-v<n>.md`, fenced JSON block).
- Frontmatter update on `hypotheses/<slug>/hypothesis.md` (`latest_ar` only — Pass 4 no longer touches `latest_wiki_html`).
...
- **Do not generate `plan/wiki.html`, `plan/index.html`, or any other static webpage.** The live AR viewer in `components/ar/` is the only renderer.
